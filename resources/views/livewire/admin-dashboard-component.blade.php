<div class="bg-base-200 px-4 py-4">
    ...
    <div class="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        {{-- Stock Alerts --}}
        @if(!empty($dashboardStats['products']['low_stock']) || !empty($dashboardStats['products']['out_of_stock']))
            <div class="card bg-base-300 shadow-lg">
                <div class="card-body">
                    <h3 class="card-title text-lg mb-4">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="space-y-3">
                        @if(!empty($dashboardStats['products']['out_of_stock']))
                            <div class="alert alert-error">
                                <svg class="w-6 h-6 shrink-0 stroke-current" fill="none" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                                </svg>
                                <div>
                                    <h4 class="font-bold">Stok Habis ({{ $dashboardStats['products']['out_of_stock']->count() }})</h4>
                                    <div class="text-sm">
                                        @foreach($dashboardStats['products']['out_of_stock']->take(3) as $product)
                                            {{ $product->name }}@if(!$loop->last), @endif
                                        @endforeach
                                        @if($dashboardStats['products']['out_of_stock']->count() > 3)
                                            dan {{ $dashboardStats['products']['out_of_stock']->count() - 3 }} lainnya
                                        @endif
                                    </div>
                                </div>
                            </div>
                        ...
                        @if(!empty($dashboardStats['products']['low_stock']))
                            <div class="alert alert-warning">
                                <svg class="w-6 h-6 shrink-0 stroke-current" fill="none" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.732 16.5c-.77.833.192 2.5 1.732 2.5z"></path>
                                </svg>
                                <div>
                                    <h4 class="font-bold">Stok Menipis ({{ $dashboardStats['products']['low_stock']->count() }})</h4>
                                    <div class="text-sm">
                                        @foreach($dashboardStats['products']['low_stock']->take(3) as $product)
                                            {{ $product->name }}@if(!$loop->last), @endif
                                        @endforeach
                                        @if($dashboardStats['products']['low_stock']->count() > 3)
                                            dan {{ $dashboardStats['products']['low_stock']->count() - 3 }} lainnya
                                        @endif
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="text-center">
                            <a href="{{ route('staf.stock-sate') }}" class="btn btn-outline btn-sm">
                                Kelola Stok
                            </a>
                        </div>
                    </div>
    ...
    function initializeCharts() {
        ...
        if (dailySalesCtx) {
            dailySalesChart = new Chart(dailySalesCtx, {
                type: 'line',
                data: {
                    labels: chartData.dailySales.map(item => item.label),
                    datasets: [{
                        label: 'Penjualan',
                        data: chartData.dailySales.map(item => item.sales),
        ...
        const hourlyPatternCtx = document.getElementById('hourlyPatternChart');
        if (hourlyPatternCtx) {
            hourlyPatternChart = new Chart(hourlyPatternCtx, {
                type: 'bar',
                data: {
                    labels: chartData.hourlyPattern.map(item => item.label),
                    datasets: [{
                        label: 'Transaksi',
                        data: chartData.hourlyPattern.map(item => item.transactions),
        ...
        if (orderTypeCtx && chartData.salesByOrderType.data.length > 0) {
            orderTypeChart = new Chart(orderTypeCtx, {
                type: 'doughnut',
                data: {
                    labels: chartData.salesByOrderType.labels,
                    datasets: [{
                        data: chartData.salesByOrderType.data,
                        backgroundColor: chartData.salesByOrderType.backgroundColor
                    }]
                },
                options: {
                    responsive: true,
        ...
        if (paymentMethodCtx && chartData.salesByPayment.data.length > 0) {
            paymentMethodChart = new Chart(paymentMethodCtx, {
                type: 'doughnut',
                data: {
                    labels: chartData.salesByPayment.labels,
                    datasets: [{
                        data: chartData.salesByPayment.data,
                        backgroundColor: chartData.salesByPayment.backgroundColor
                    }]
                },
                options: {
                    responsive: true,
        ...
        if (expensesCategoryCtx && chartData.expensesByCategory.data.length > 0) {
            expensesCategoryChart = new Chart(expensesCategoryCtx, {
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        backgroundColor: chartData.expensesByCategory.backgroundColor
                    }]
                },
                options: {
                    responsive: true,
